...
bool development_imu = false;
void imu_odometry(void)
{ // imu odom, expected high standard deviation, experimental
  static float T = 0;
  static double previousT = 0;
  T = float(millis()) / 1000 - previousT;
  previousT += T;
  double i1 = inertial.get_accel().x -i1offset;
  double i2 = inertial2.get_accel().x-i2offset;
  double i3 = inertial3.get_accel().x-i3offset;
  double j1 = inertial.get_accel().y -j1offset;
  double j2 = inertial2.get_accel().y-j2offset;
  double j3 = inertial3.get_accel().y-j3offset;
  double k1 = inertial.get_accel().z -k1offset;
  double k2 = inertial2.get_accel().z-k2offset;
  double k3 = inertial3.get_accel().z-k3offset;
  // step one: data cleaning
  //  clean out gravitational acceleration
  // step two: integration
  if (development_imu)
  {
    sprintf(outNames[0], "time");
    outVals[0] = previousT;
    sprintf(outNames[1], "i2");
    outVals[1] = i2;
    sprintf(outNames[2], "i3");
    outVals[2] = i3;
    sprintf(outNames[3], "j2");
    outVals[3] = j2;
    sprintf(outNames[4], "j3");
    outVals[4] = j3;
    sprintf(outNames[5], "k2");
    outVals[5] = k2;
    sprintf(outNames[6], "k3");
    outVals[6] = k3;
  }
}

void enc_odometry(void)
{ // encoder odom, primary odom,
  ...
  double prev_Vx, prev_Vy, prev_W, Accx, Accy, Accw;
  ...
  if (P1 != 0)
  {                                                    // if there are change of heading while moving, arc approximation
  ...
  T = float(millis()) / 1000 - previousT;
  previousT += T;
  Accx = -(robot.xVelocity - Delta_x / T) / T;
  Accy = -(robot.yVelocity - Delta_y / T) / T;
  Accw = -(robot.wVelocity - Delta_heading / T) / T;
...
void mot_odometry(void)
{ // encoder odom, primary odom,
  static double odomposx = 0;
  static double odomposy = 0;
  double Arc1 = distTraveled(&rightEncoderFB); // rightEncoderFB travel, to forward direction of robot is positive
  double Arc2 = distTraveled(&leftEncoderFB);  // leftEncoderFB travel, to forward direction of robot is positiv
  double a = 8.5;                              // distance between two tracking wheels
  double P1 = (Arc1 - Arc2);
  double Delta_y, Delta_x;
  double radRotation = mod(2 * M_PI, (-inertial.get_heading() + robot.chaIntAng) * M_PI / 180);
  double prev_Vx, prev_Vy, prev_W, Accx, Accy, Accw;
  ...
  {                                                    // if there are change of heading while moving, arc approximation
    ...
    double cos_side = sin(odoHeading + Delta_heading) - sin(odoHeading);
    double sin_side = -cos(odoHeading + Delta_heading) + cos(odoHeading);
    Delta_x = Radius_side * cos_side;
    Delta_y = Radius_side * sin_side;
  }
  else
  {
    Delta_x = Arc1 * cos(odoHeading);
    Delta_y = Arc1 * sin(odoHeading);
  }
  odoHeading += Delta_heading;
  odoHeading = mod(2 * M_PI, odoHeading);
  robot.angle = odoHeading * 180 / M_PI;
  ...
  sprintf(outNames[0], "time");
  outVals[0] = previousT;
  sprintf(outNames[1], "vx");
  outVals[1] = robot.xVelocity;
  sprintf(outNames[2], "vy");
  outVals[2] = robot.yVelocity;
  sprintf(outNames[3], "accx1");
  outVals[3] = Accx;
  sprintf(outNames[4], "accy1");
  outVals[4] = Accy;
  sprintf(outNames[5], "x");
  outVals[5] = robot.xpos;
  sprintf(outNames[6], "y");
  outVals[6] = robot.ypos;
  sprintf(outNames[7], "_dt");
  outVals[7] = T;
}
void turAngupdate()
{
